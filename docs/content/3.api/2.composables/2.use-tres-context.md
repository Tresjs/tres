...
::warning
`useTresContext` can only be used inside of a `TresCanvas` component since it relies on the TresJS context provider.
::
...
onMounted(() => {
  // Create multiple cameras with different positions
  const camera1 = new PerspectiveCamera(75, sizes.aspectRatio.value, 0.1, 1000)
  camera1.position.set(5, 5, 5)
  camera1.lookAt(0, 0, 0)

  const camera2 = new PerspectiveCamera(50, sizes.aspectRatio.value, 0.1, 1000)
  camera2.position.set(-5, 2, 5)
  camera2.lookAt(0, 0, 0)

  const camera3 = new PerspectiveCamera(90, sizes.aspectRatio.value, 0.1, 1000)
  camera3.position.set(0, 10, 0)
  camera3.lookAt(0, 0, 0)

  // Register all cameras
  camera.registerCamera(camera1, true) // Set first as active
  camera.registerCamera(camera2)
  camera.registerCamera(camera3)
})

const switchCamera = () => {
  const cameras = camera.cameras.value
  currentCameraIndex.value = (currentCameraIndex.value + 1) % cameras.length
  camera.setActiveCamera(cameras[currentCameraIndex.value])
}
</script>

<template>
  <TresMesh @click="switchCamera">
    <TresBoxGeometry />
    <TresMeshNormalMaterial />
  </TresMesh>
</template>
...
::tip
**Developer Tip:**
Choose `useTresContext` for advanced 3D applications, plugin development, or when you need complete control over the rendering pipeline. Use `useTres` for typical 3D scene interactions.
::
