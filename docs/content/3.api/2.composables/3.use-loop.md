...
::warning
**Success Callback Required**: You must call the `notifySuccess()` callback at the end of your render function to properly notify the render loop that the frame was completed. This is essential for render modes (`always`, `on-demand`, `manual`) to function correctly.
::
...
- Manually calling `renderer.instance.render(scene.value, camera.activeCamera.value)`
