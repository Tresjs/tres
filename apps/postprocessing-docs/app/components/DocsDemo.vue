...
const demoName = computed(() => {
  ...
  // meta is keyed by PascalCase, the tag is kebab-case (pmndrs-bloom -> PmndrsBloom)
  return tag.split('-').map(part => part.charAt(0).toUpperCase() + part.slice(1)).join('')
})

const demoSources = import.meta.glob(
  [
    '../../app/components/pmndrs/*.vue',
    '../../app/components/three/*.vue',
  ],
...
const { data: demoData } = useAsyncData(
  ...
  async () => {
    ...
    if (!name) {
      console.warn('[DocsDemo] could not resolve demo component name from slot')
    ...
    if (!filePath) {
      console.warn(`[DocsDemo] no meta found for demo "${name}"`)
      return { filePath: undefined, raw: undefined }
    }
    const marker = filePath.includes('/pmndrs/') ? 'pmndrs/' : 'three/'
    const key = `./${filePath.slice(filePath.indexOf(marker))}`
    const loader = demoSources[key]
    if (!loader) {
      console.warn(`[DocsDemo] no source found for demo "${name}" (key "${key}")`)
...
const sourceUrl = computed(() => {
  ...
  return `https://github.com/Tresjs/tres/blob/main/apps/postprocessing-docs/${filePath}`
})
