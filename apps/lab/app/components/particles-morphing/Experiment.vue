...
const { nodes: blenderCubeNodes } = useGLTF('/models/blender-cube-simplified.glb', {
  draco: true,
})
const blenderCube = computed(() => blenderCubeNodes.value?.BlenderCube)

const { nodes: pumpkinsaurNodes } = useGLTF('/models/pumpkinsaur-simplified.glb', {
  draco: true,
})
const pumpkinsaur = computed(() => pumpkinsaurNodes.value?.Pumpkinsaur)

watch(
  () => [blenderCube.value, pumpkinsaur.value],
  ([blenderCube, pumpkinsaur]) => {
    if (blenderCube && pumpkinsaur) {
      ...
      particles.maxCount = Math.max(
        blenderCube.geometry.attributes.position.count,
        pumpkinsaur.geometry.attributes.position.count,
      )
