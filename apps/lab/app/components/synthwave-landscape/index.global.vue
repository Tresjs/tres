...
const terrainGen = (planeX: number, planeY: number): [number, number, number] => {
  ...
  const noiseV = Math.abs(
    noise(
      planeX * NOISE_SCALE,
      pingpong(planeY * 4, 0.5) * (NOISE_SCALE * (2 * Math.sin(PI2 * planeY))),
      0,
    ),
  )
  const noisePeaks = -(NOISE_PEAK + Math.abs(Math.sin(PI2 * planeY))) * noiseV
  const y =
    (noisePeaks + noisePeaks * Math.abs(wave)) * Math.abs(Math.sin(PI2 * (planeX - path))) + wave
  return [-x, -y, z]
}
...
<template>
  ...
  <TresCanvas v-bind="gl" :disable-render="false" @loop="onLoop">
    ...
    <TresPerspectiveCamera :position="[x, y, z]">
      ...
      <SynthwaveLandscapeMountain
        :color="PALETTE[8]"
        :position="[0, 0, -500]"
        :scale="[24, 24, 12]"
      />

      <SynthwaveLandscapeSun
        :scale="100"
        :color-a="PALETTE[3]"
        :color-b="PALETTE[7]"
        :position="[0, 133, -600]"
