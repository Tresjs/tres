...
const { state: boneTexture, isReady: isBoneTextureReady } = useTexture(
  '/models/potions-classroom/skull.png',
)

const skulls = computed(() =>
  Object.values(nodes.value).filter((node) => node.name.includes('Skull')),
)
