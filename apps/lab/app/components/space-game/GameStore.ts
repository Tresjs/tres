  ...
  if (mutation.hits && lasers.length && time - lasers[lasers.length - 1] < 100) {
    ...
    allHit.forEach(data => updates.push({
      ...
      particles: Array.from({ length: 20 }).fill(0).map(_ => ({
        position: new Vector3(),
        dPos: new Vector3(-1 + Math.random() * 2, -1 + Math.random() * 2, -1 + Math.random() * 2).normalize().multiplyScalar(0.40),
      })),
    }))
    allHit.forEach(data => updates.push({
      time: Date.now(),
      ...
      particles: Array.from({ length: 20 }).fill(0).map(_ => ({
        position: new Vector3(),
        dPos: new Vector3(-1 + Math.random() * 2, -1 + Math.random() * 2, -1 + Math.random() * 2).normalize().multiplyScalar(0.60),
      })),
    }))
    ...
    gameStore.rocks = gameStore.rocks.filter(rock => !rocksHit.some(r => r.guid === rock.guid))
    gameStore.enemies = gameStore.enemies.filter(enemy => !enemiesHit.some(e => e.guid === enemy.guid))
  }
}
