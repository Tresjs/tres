...
onBeforeRender(({ elapsed }) => {
  if (shaderToyTarget.value && shaderToyTarget.value.lights) {
    for (const light of shaderToyTarget.value.lights) {
      if (light.userData.x) {
        light.position.x =
          light.userData.x.init +
          Math.cos(light.userData.x.speed * elapsed + light.userData.x.phase) *
            light.userData.x.dist
      }
      if (light.userData.y) {
        light.position.y =
          light.userData.y.init +
          Math.cos(light.userData.y.speed * elapsed + light.userData.y.phase) *
            light.userData.y.dist
      }
      if (light.userData.z) {
        light.position.z =
          light.userData.z.init +
          Math.cos(light.userData.z.speed * elapsed + light.userData.z.phase) *
            light.userData.z.dist
      }
      if (light.userData.intensity) {
        light.intensity = lerp(
          light.userData.intensity.a,
          light.userData.intensity.b,
          pingpong(elapsed * light.userData.intensity.speed),
        )
      }
