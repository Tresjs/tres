...
const { state: model, isLoading, error, progress } = useLoader(GLTFLoader, '/models/duck.gltf')
...
const {
  ...
  isLoading,
  progress,
} = useLoader(GLTFLoader, '/models/compressed.glb', {
  extensions: (loader) => {
...
const { state: texture, isLoading } = useLoader(TextureLoader, 'https://example.com/texture.jpg')
```
...
<template>
  <TresCanvas :shadows="shadowsEnabled" :tone-mapping="currentToneMapping">
    <!-- Your scene -->
  </TresCanvas>
</template>
```
