...
const demoName = computed(() => {
  ...
  // meta is keyed by PascalCase, the tag is kebab-case (examples-my-first-scene -> ExamplesMyFirstScene)
  return tag.split('-').map(part => part.charAt(0).toUpperCase() + part.slice(1)).join('')
})

const demoSources = import.meta.glob('../../app/components/examples/*/*.vue', {
  query: '?raw',
  import: 'default',
...
const { data: demoData } = useAsyncData(
  ...
  async () => {
    ...
    const key = `./${filePath.slice(filePath.indexOf('examples/'))}`
...
const sourceUrl = computed(() => {
  ...
  return `https://github.com/Tresjs/tres/blob/main/apps/docs/${filePath}`
})
