...
<template>
  ...
  <OverlayInfo>
    ...
    <p>
      This clamps the possible range for the renderer's DPR setting. (TresCanvas also accepts a
      numerical value for <code>:dpr</code>. That is not tested in this page.)
    </p>
    <h2>Try it</h2>
    <p>
      Zooming in and out in the browser
      <a href="https://developer.mozilla.org/en-US/docs/Web/API/Window/devicePixelRatio"
        >triggers a change to the window's <code>devicePixelRatio</code>.</a
      >
      In turn, this should set the renderer's DPR. It should remain within the range specified by
      the <code>:dpr</code> prop.
    </p>
    <h2>Test</h2>
    <p>
      Renderer DPR: <span>{{ currDprRef }}</span>
    </p>
    <p v-if="!rendererRef || isRendererDprClamped(rendererRef)" :style="{ color: 'green' }">
      ✅ DPR is clamped.
    </p>
    <p v-else :style="{ color: 'red' }">❌ DPR is not properly clamped.</p>
  </OverlayInfo>
</template>
