...
pane.addBinding({ value: waterColor.value }, 'value', {
    label: 'waterColor',
    view: 'color',
}).on('change', (ev) => (waterColor.value = ev.value))

pane.addBinding({ value: sunColor.value }, 'value', {
    label: 'sunColor',
    view: 'color',
}).on('change', (ev) => (sunColor.value = ev.value))

pane.addBinding({ value: speed.value }, 'value', {
    label: 'speed',
    min: 0,
    max: 1,
    step: 0.01,
...
pane.addBinding({ value: size.value }, 'value', {
    label: 'size',
    min: 1,
    max: 50,
    step: 1,
...
pane.addBinding({ value: distortionScale.value }, 'value', {
...
pane.addBinding({ value: alpha.value }, 'value', {
    label: 'alpha',
    min: 0,
    max: 1,
    step: 0.01,
...
<template>
    <TresCanvas v-bind="gl">
        ...
        <OrbitControls :enable-pan="false" :enable-zoom="false" :max-polar-angle="Math.PI * 0.495" :min-distance="40.0" :max-distance="200.0" />
    </TresCanvas>
</template>
