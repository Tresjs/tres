...
export const createColliderDesc = (props: CreateColliderDescProps) => {
  ...
  let colliderDesc = ColliderDesc.cuboid(halfWidth ?? 1, halfHeight ?? 1, halfDepth ?? 1)
  ...
  colliderDesc.setTranslation(position.x, position.y, position.z).setRotation(rotation)
...
export const createColliderPropsFromObject = (
  ...
  return {
    shape,
    object,
    args: shape === 'ball' ? [sizes.radius] : [sizes.halfWidth, sizes.halfHeight, sizes.halfDepth],
