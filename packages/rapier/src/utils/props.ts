...
export const makePropWatcherRB = <K extends keyof RigidBodyProps>(
  ...
  watch([() => props[toWatch], instance], ([newValue, _]) => {
    ...
    ;(
      instance.value[
        onSet as keyof Methods<RigidBody>
      ] as CallableProps<RigidBody>[keyof CallableProps<RigidBody>]
    )?.(...(Array.isArray(newValue) ? (newValue as boolean[]) : [newValue]), true)
  })

export const makePropsWatcherRB = <K extends keyof RigidBodyProps>(
  props: RigidBodyProps,
  watchers: K[],
  instance: ShallowRef<RigidBodyContext['rigidBody'] | undefined>,
) =>
  watchers.forEach((_) => {
    ...
    const watcherName = (watcher.charAt(0).toUpperCase() + watcher.slice(1)) as Capitalize<
      keyof RigidBodyProps
    >
    makePropWatcherRB(props, watcher as keyof RigidBodyProps, instance, `set${watcherName}`)
  })

export const makePropWatcherCL = <K extends keyof ColliderProps>(
  props: ColliderProps,
  toWatch: K,
  ...
  watch([() => props[toWatch], instance], ([newValue, _]) => {
    ...
    ;(
      ...
      ] as CallableProps<ColliderProps>[keyof CallableProps<ColliderProps>]
    )?.(newValue, true)
  })

export const makePropsWatcherCL = <K extends keyof ColliderProps>(
  props: ColliderProps,
  watchers: K[],
  instance: ShallowRef<CreateColliderReturnType | undefined>,
) =>
