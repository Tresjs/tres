...
const pointsF = computed(() =>
  props.points
    .map((point, index) => `${index * strokeWidth},${height - (point * height) / maxValue.value}`)
    .join(' '),
)
</script>
...
<template>
  <div
    class="graph relative p-1 rounded text-right text-xs outline-none border-none font-sans"
    :class="`graph-${color}`"
  >
    <div class="absolute bottom-0.5 right-1 font-mono text-xs" :class="`text-${color}`">
