...
export type DecalEventPayload =
  ...
  | { type: 'click', textureCount: number }
  ...
  | { type: 'validate-decal' }
  | { type: 'set-ui-state', scale: number, angle: number, x: number, y: number, id: string, textureCount: number }
  | { type: 'cancel-edit' }
  | { type: 'change-zindex', id: string, delta: number }
  | { type: 'refresh-raycasts' }
  | { type: 'delete-decal' }
  | { type: 'change-texture', direction: -1 | 1 }
  | { type: 'request-export' }
