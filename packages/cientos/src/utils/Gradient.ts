...
export function normalizeColorGradient(gradient: GradientTresColor): NormalizedGradient<Color> {
...
function isVectorFlexibleParams(p: any) {
  return (
    'isVector3' in p || (Array.isArray(p) && p.length > 0 && p.every((v) => typeof v === 'number'))
  )
}

export function normalizeFlexibleVector3Gradient(gradient: GradientVectorFlexibleParams) {
  return normalizeGradient<VectorFlexibleParams, Array<number>>(gradient, {
    normalizeValue: (input: VectorFlexibleParams) => normalizeVectorFlexibleParam(input),
...
export function normalizeScalarGradient(gradient: GradientScalar): NormalizedGradient<number> {
  return normalizeGradient<number, number>(gradient, {
    normalizeValue: (input: number) => input,
    getDefaultValue: () => 1,
    isSingleValue: (t: typeof gradient) => !Array.isArray(t) && typeof t !== 'undefined',
    isMultipleValues: (t: typeof gradient) =>
      Array.isArray(t) && (t.length === 0 || !Array.isArray(t[0])),
    isMultipleValuesWithStops: (t: typeof gradient) =>
      Array.isArray(t) && t.length > 0 && Array.isArray(t[0]),
...
function normalizeGradient<T, U>(
  ...
  const isMultipleValues = (t: Gradient<T>): t is T[] => config.isMultipleValues(t)
